"""End-to-end tests for the Hindsight-OpenAI-Agents integration.
...
The whole module is the real-LLM bucket (``requires_real_llm``): excluded from
PR CI via ``-m "not requires_real_llm"``; selectable via ``-m requires_real_llm``.
"""
...
async def _run(tool, args: dict) -> str:
    ctx = MagicMock()
    return str(await tool.on_invoke_tool(ctx, json.dumps(args)))


async def _recall_until_nonempty(recall_tool, query, attempts=12, delay=1.0):
    """Poll recall until it surfaces a memory (retain takes a moment to index)."""
...
async def live():
    ...
    bank_id = f"openai-agents-e2e-{uuid.uuid4().hex[:8]}"
    await client.acreate_bank(bank_id, name=f"OpenAI Agents E2E {bank_id}")
    try:
...
class TestE2EMemoryInstructions:
    @pytest.mark.asyncio
    async def test_injects_recalled_memory(self, live):
        client, bank_id = live
        ...
        result = None
        for _ in range(12):
            result = await get_instructions(MagicMock(), MagicMock())
