"""Configuration management for the Hindsight GitHub Copilot CLI integration.

Loading order (later entries win):
  1. Built-in defaults
  2. Plugin install settings.json  (~/.copilot/hooks/hindsight-copilot-cli/settings.json)
  3. User config                   (~/.hindsight/copilot-cli.json)
  4. Environment variable overrides

``~/.hindsight/copilot-cli.json`` is the recommended place to configure the
...
DEFAULTS = {
    # Recall (sessionStart / subagentStart only — see hooks/README for why
    # Copilot CLI can't support per-prompt recall the way other tools do)
    "autoRecall": True,
    "recallBudget": "mid",
    ...
    # Fallback query used when a hook has no specific prompt/task text to
    # recall against (e.g. an interactive sessionStart with no initialPrompt,
    # or any subagentStart — the payload never carries per-invocation task
    # text). {project} is replaced with the cwd's basename.
    "recallFallbackQueryTemplate": (
        "Project: {project}. Recall recent decisions, conventions, "
        "preferences, and context relevant to continuing work here."
    ),
    # Retain
    "autoRetain": True,
    ...
    "bankMission": (
        "You are a GitHub Copilot CLI coding assistant. Focus on technical "
...
def load_config():
    ...
    user_config_path = os.path.join(os.path.expanduser("~"), ".hindsight", "copilot-cli.json")
