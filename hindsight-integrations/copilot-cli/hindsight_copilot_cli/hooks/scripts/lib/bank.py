...
Copilot CLI context dimensions:
  - agent   → configured name or "copilot-cli" (HINDSIGHT_AGENT_NAME)
  - project → derived from the hook payload's `cwd` field (Copilot CLI has no
    workspace_roots equivalent — every hook payload carries a single cwd)
  - user    → from env var HINDSIGHT_USER_ID

Like Cursor CLI, Copilot CLI's `agentStop`/`sessionEnd` hooks aren't scoped to
a specific transcript turn beyond the payload's own sessionId, so "session" is
exposed as an opt-in granularity field rather than a default.

The channel dimension is omitted — Copilot CLI is a CLI tool without
...
from .hook_io import field as hook_field
from .state import read_state, write_state

DEFAULT_BANK_NAME = "copilot-cli"

# Valid granularity fields for the Copilot CLI integration.
VALID_FIELDS = {"agent", "project", "session", "user"}


def _resolve_project_name(hook_input):
    """Resolve the project name for bank-id derivation from the hook's `cwd`.

    Returns "unknown" when no cwd is available so bank IDs stay stable across
    test runners (avoids leaking the test runner's cwd into the bank name).
    """
...
def _resolve_session_id(hook_input):
    ...
    return hook_field(hook_input, "sessionId", "session_id", default="unknown") or "unknown"
    ...
    for f in fields:
        if f not in VALID_FIELDS:
            print(
                ...
                f"valid for copilot-cli: {', '.join(sorted(VALID_FIELDS))}",
                file=sys.stderr,
            )

    agent_name = config.get("agentName", "copilot-cli")
