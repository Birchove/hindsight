"""Tests for lib/content.py — content processing for the Copilot CLI integration."""
...
class TestReadTranscript:
    ...
    def test_reads_sdk_envelope(self, tmp_path):
        f = tmp_path / "transcript.jsonl"
        f.write_text(
            "\n".join(
                [
    ...
    def test_reads_role_nested_agent_transcript(self, tmp_path):
        """Some tool transcript formats use role-nested block content (as
        opposed to the type-nested SDK envelope) — `content.py`'s parser
        supports both defensively for robustness across transcript formats.
        """
        f = tmp_path / "role-nested.jsonl"
        f.write_text(
            "\n".join(
                [
                    json.dumps(
